...
contract SingleInvoker is BaseInvoker {
    ...
    /// @notice thrown when an transaction is executed with an invalid nonce
    ...
    /// @notice execute a transaction behalf of a signing authority using AUTH and AUTHCALL
    ...
    function exec(address authority, bytes memory execData) internal override {
        (uint256 nonce, uint256 gas, uint256 value, address to, bytes memory data) = abi.decode(execData, (uint256, uint256, uint256, address, bytes));
